...
//push
bool empilhaP(Pilha *p, Pessoa dado)
{
...
void mostraP(Pilha *p, string nome="")
{
    ...
    if(vaziaP(p) == false)
    {
        ...
        while (no != NULL)
        {
            ...
            cout << "["<< no->dado.nome << ", "  << no->dado.peso << ", "  << no->dado.altura << "]";
